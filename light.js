...
/* =========================================================
   ★ Directive 2 — pop.png popup + 10s countdown (pop.png fill)
========================================================= */
let popupEl = null;
...
function showPopup() {
  ...
  const popTimer = setInterval(() => {
    if (!popupEl) { clearInterval(popTimer); return; }
    if (created >= POP.maxCount) { clearInterval(popTimer); return; }

    for (let i = 0; i < POP.burstCount; i++) {
      if (created >= POP.maxCount) break;
      ...
      const size = POP.minSize + Math.random() * (POP.maxSize - POP.minSize);
  ...
  if (currentStage === 1) {
    ...
    showGuideOverlay(`
      <div style="
        ...
        text-align:center;
      ">
        <div style="color:rgba(255,255,255,0.9);font-size:15px;letter-spacing:0.18em;text-shadow:0 0 20px rgba(255,255,255,0.6);white-space:nowrap;">
          CLICK A SHAPE
        </div>
      </div>
    `);
...
function startOpening() {
  ...
  // I UNDERSTAND button (white + bigger)
  ...
  understandBtn.style.cssText = `
    ...
    letter-spacing:0.12em;padding:13px 36px;cursor:pointer;
    transition:background 0.2s;
    white-space:nowrap;
  `;
